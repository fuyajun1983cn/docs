...
** Suspend/Resume操作
    ...
    对于cob的情况，有如下两点需要注意：
    1. host端需要设置 nonremovable， 
       : mmc->caps |= MMC_CAP_NONREMOVABLE
       如果没有设置的话， resume的时候会跑进detect， 检测到卡被移除了，
       从而执行了移除操作。 
    2. host端需要设置keep power. driver suspend时保持SDIO BUS处于上电状态：
       : sdio_set_host_pm_flags(func, MMC_PM_KEEP_POWER)
       如果没有设置，sdio在resume 的时候就会跑sdio reset和go idle
       （CMD0）后进入 =init_card= , 第一个CMD就不会响应了。
